...
if [ -e "/tmp/DeployStudioServer.pkg" ]
then
  rm -rf "/tmp/DeployStudioServer.pkg"
fi
...
then
  PKG_FILE_NAME=`unzip -l "${INSTALLER_ARCHIVE_PATH}" | grep ".pkg/$" | awk '{ print $4 }' | sed s+/$++`
  if [ -z "${PKG_FILE_NAME}" ]
  then
  ...
  if [ ${?} -eq 0 ]
  then
    mv "/tmp/${PKG_FILE_NAME}" "/tmp/DeployStudioServer.pkg"
...
then
  ...
  then
    ...
    then
	  PACKAGE_NAME=`ls "/Volumes/${INSTALLER_VOLUME}" | grep "^DeployStudioServer_.*.pkg$" | head -n 1`
	  if [ -n "${PACKAGE_NAME}" ]
	  then
		...
		then
          ...
          ditto --rsrc "${PACKAGE_PATH}" "/tmp/DeployStudioServer.pkg" 2>&1
		fi
      else
